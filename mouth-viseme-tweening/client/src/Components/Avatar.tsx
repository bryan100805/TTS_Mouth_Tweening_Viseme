...
function Avatar(props: SVGProps<SVGSVGElement>) {
  ...
  const interpolateAndSetViseme = async (startId: number, endId: number) => {
    ...
    for (let i = 0; i < numFrames; i++) {
      frames.push(createFrameSvg(startPaths, endPaths, i / numFrames));
    }

    // Optional: handle the frames, e.g., display or download
    console.log(frames);

  };

  const createFrameSvg = (startPaths:string[], endPaths:string[], t:number) => {
  ...
  const createSvgContent = (paths: string[]) => {
    console.log(`Paths to be SVG`, paths)
    const xmlHeader = `<?xml version="1.0" encoding="UTF-8"?>`
    const svgHeader = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080">`;
    const pathElements = paths.map(path => `<path d="${path}" fill="#801817" stroke="black"/>`).join('');
    const svgFooter = `</svg>`;
    return xmlHeader+svgHeader + pathElements + svgFooter;
  };
